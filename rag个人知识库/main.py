...
from rag个人知识库.demo import save_chunks, search_with_rerank
...
for i, doc in enumerate(docs, start=1):
    if isinstance(doc, DocumentValidationError):
        print(f"第{i}个文档{doc.file_path}加载失败,失败原因:{doc.error_msg}")
        print("="*30)
    else :
        print(f"第{i}个文档{doc[0].metadata['source']}加载成功")
        print("=" * 30)
        for page in doc[0:5]:
        ...
        for i, chunk in enumerate(chunks, start=1):
            print(f"第{i}个分块内容：{chunk.page_content[:200]}")
            print(f"第{i}个分块元数据：{chunk.metadata}")
        # 切分完成后向量化入库：确定性 ID 保证重复运行不产生重复向量
        print("=" * 30)
        save_chunks(chunks)
    print(type(doc))

# 入库完成后做一次两段式检索：向量召回 Top20 → bge-reranker 精排取 Top3
